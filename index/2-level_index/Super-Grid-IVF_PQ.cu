...
namespace super_grid {
    ...
    {   
        ...
        params.n_clusters = super_grid_config.clusters;
        ...
        parafilter_mmr::set_tag(MEM_INDEX);
#endif
        centers_view = parafilter_mmr::make_device_matrix_view<float, uint64_t>(params.n_clusters, n_dim);
#ifdef TAGGED_MMR
        parafilter_mmr::set_tag(MEM_DEFAULT);
#endif
        ...
        parafilter_mmr::set_tag(MEM_INDEX);
#endif
        float* secondary_centers = static_cast<float*>(parafilter_mmr::mem_allocator(clusters * sub_clusters * n_dim * sizeof(float)));
        ...
        for (auto &cluster : grouped_clusters) {
            uint64_t cur_cluster_len = cluster.extent(0);
            if (cur_cluster_len == 0) {
                thrust::fill_n(thrust::device_pointer_cast(centers_view.data_handle()) + n_dim * center_id, n_dim, 1e6 / n_dim);
                center_id++;
                continue;
            }
            params.n_clusters = std::min(uint64_t(sub_clusters), cur_cluster_len);
            ...
            auto indices_view = raft::make_device_matrix_view<uint64_t, uint64_t>(cluster.data_handle(), cur_cluster_len, 1);
            ...
            sub_cluster_offsets_host[center_id] = tot_centers;
            sub_cluster_counts_host[center_id] = params.n_clusters;
            ...
            center_id++;
            int sub_id = 0;
            for (auto sub_cluster : cur_secondary_clusters_list) {
                cluster_data sub_cluster_data;
                uint64_t sub_cluster_len = sub_cluster.extent(0);
                /*processing null sub-clusters*/
                if (sub_cluster_len == 0) {
                    ...
                    thrust::fill_n(thrust::device_pointer_cast(cur_centers_view.data_handle()) + n_dim * center_id, n_dim, 1e6 / n_dim);
                    sub_center_id++;
                    sub_id++;
                    continue;
                }
                ...
                assert(zorder_pool_offset + sub_cluster_len <= n_data);
                
                auto data_ids = raft::make_device_vector_view<uint64_t, uint64_t>(zcode_ids_pool + zorder_pool_offset, sub_cluster_len);
                select_elements(in.dev_resources, cluster, sub_cluster, data_ids);
                
                auto grid_ids = parafilter_mmr::make_device_vector_view<uint64_t, uint64_t>(sub_cluster_len);
                auto grid_coords = parafilter_mmr::make_device_matrix_view<int, uint64_t>(sub_cluster_len, grid_dims_host.size());
                ...
                thrust::sort(
                    ...
                    thrust::device_pointer_cast(grid_ids.data_handle()) + sub_cluster_len
                ); 
        ...
        parafilter_mmr::set_tag(MEM_DEFAULT);
                ...
                raft::device_vector_view<uint64_t, uint64_t> zcodes = raft::make_device_vector_view<uint64_t, uint64_t>(zorder_code_pool + zorder_pool_offset, sub_cluster_len);
                encode_points(grid_coords, zcodes, morton_config);

                thrust::sort_by_key(
                    thrust::device_pointer_cast(zcodes.data_handle()),      
                    thrust::device_pointer_cast(zcodes.data_handle())  + sub_cluster_len, 
                    thrust::device_pointer_cast(data_ids.data_handle())   
                );   
                
                int my_array[] = {271857, 16748, 249565, 217064, 249554, 249451, 249441, 249439, 79954};
                for (int i = 0; i < 9; i++) {
                    auto p = thrust::find(thrust::device, data_ids.data_handle(), data_ids.data_handle() + sub_cluster_len, my_array[i]);
                    if (p != data_ids.data_handle() + sub_cluster_len) {
                        LOG(TRACE) << "find " << my_array[i] << " in cluster" << sub_center_id << "offset: " << p - data_ids.data_handle() << " pool offset: " << zorder_pool_offset;
                    }
                }

                zorder_offsets_host[sub_center_id] = zorder_pool_offset;
                zorder_lengthes_host[sub_center_id] = sub_cluster_len;
                
                zorder_pool_offset += sub_cluster_len;
                sub_center_id++;
            }
        }

        assert(zorder_pool_offset == n_data);
        super_grid_index_data.sub_clusters_data.sub_centers = raft::make_device_matrix_view<float, uint64_t>(secondary_centers, tot_centers, n_dim);

        cudaMemcpy(zorder_offsets, zorder_offsets_host, sizeof(uint64_t) * sub_center_id, cudaMemcpyHostToDevice);
        super_grid_index_data.sub_clusters_data.zorder_array_offsets = raft::make_device_vector_view<uint64_t, uint64_t>(zorder_offsets, sub_center_id);

        cudaMemcpy(zorder_lengthes, zorder_lengthes_host, sizeof(uint64_t) * sub_center_id, cudaMemcpyHostToDevice);
        super_grid_index_data.sub_clusters_data.zorder_array_lengthes = raft::make_device_vector_view<uint64_t, uint64_t>(zorder_lengthes, sub_center_id);

        cudaMemcpy(sums_device, sums_host, sizeof(prefix_sum_node*) * sub_center_id, cudaMemcpyHostToDevice);
        super_grid_index_data.sub_clusters_data.compressed_sums = raft::make_device_vector_view<prefix_sum_node*, uint64_t>(sums_device, sub_center_id);

        cudaMemcpy(sub_cluster_offsets, sub_cluster_offsets_host, sizeof(uint64_t) * center_id, cudaMemcpyHostToDevice);
        cudaMemcpy(sub_cluster_counts, sub_cluster_counts_host, sizeof(uint64_t) * center_id, cudaMemcpyHostToDevice);

        super_grid_index_data.sub_cluster_offsets = raft::make_device_vector_view<uint64_t, uint64_t>(sub_cluster_offsets, center_id);
        super_grid_index_data.sub_cluster_counts = raft::make_device_vector_view<uint64_t, uint64_t>(sub_cluster_counts, center_id);
    ...
    ) 
    {
        ...
        if (sub_tree_id >= cluster_counts[tree_id]) return;

        const int* query = &queries[qid * real_dims * 2];
        int prefix_tree_offset = cluster_offsets[tree_id] + sub_tree_id;
...
#pragma unroll
    ...
    )
    {
        ...
        build_offset_table(cluster_candi_lengths, cluster_candi_starts, offset_table);
    ...
    void index::query(query_input_t const& in, query_output_t & out) const 
    {
        ...
        if (n_batch_split > 1) {
            ...
            build_offset_table<uint64_t, uint64_t>(dense_out_counts, dense_out_offsets, offset_table_null, row_lengthes);
            ...
            for (int i = 0; i < n_batch_split; i++) {
                ...
                raft::device_matrix_view<uint64_t, uint64_t> offset_table{};
                build_offset_table<uint64_t, uint64_t>(cur_counts_view, cur_offsets_view, offset_table);
            ...
            raft::device_matrix_view<uint64_t, uint64_t> offset_table{};
            build_offset_table<uint64_t, uint64_t>(dense_out_counts, dense_out_offsets, offset_table);
