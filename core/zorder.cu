...
)
{
    ...
    // statistic* z_statistics = (statistic*)parafilter_mmr::mem_allocator(sizeof(statistic) * n_queries * n_candies * n_splits);
    ...
    /*
    statistic* z_statistics_host = new statistic[n_queries * n_candies * n_splits];
    cudaDeviceSynchronize();
    cudaMemcpy(z_statistics_host, z_statistics, sizeof(statistic) * n_queries * n_candies * n_splits, cudaMemcpyDeviceToHost);

    for (int i = 0; i < n_queries; i++) {
        for (int j = 0; j < n_candies; j++) {
            for (int k = 0; k < n_splits; k++) {
                uint64_t linear_id = i * n_candies * n_splits + j * n_splits + k;

                statistic stats = z_statistics_host[linear_id];
                // printf("%d %d %d, ", stats.start, stats.count, stats.is_valid);
            }
            // printf("cani %d end\n", j);
        }
        // printf("query %d end \n", i);
