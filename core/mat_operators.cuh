...
) 
{
    ...
    auto length_sums = parafilter_mmr::make_device_matrix_view<SizeType, uint64_t>(n_queries, seg_counts);
    ...
    if (row_lengthes_optional.has_value()) {
        row_lengthes = *row_lengthes_optional;
    }
    else {
    ...
    /*if user provice row_lengthes view, then not build the table now*/
    if (row_lengthes_optional.has_value()) return;
    ...
    build_index_table_kernel<<<grids, blocks>>>(starts.data_handle(), length_sums.data_handle(), offsets_table.data_handle(), seg_counts, n_queries, table_len);
}
...
)
{
    ...
    if (index_table_optional.has_value()) {
        ...
        build_offset_table(lengthes, starts, index_table); 
    }
